...
if ( ! function_exists( 'fukasawa_load_javascript_files' ) ) {
	...
	function fukasawa_load_javascript_files() {
		...
		if ( ! is_admin() ) {
			wp_register_script( 'fukasawa_flexslider', get_template_directory_uri() . '/js/flexslider.min.js', '', true );

			//wp_enqueue_script( 'jquery-infinitescroll', get_template_directory_uri() . '/js/jquery.infinitescroll.min.js', array( 'jquery', 'masonry' ), '', true );
			wp_enqueue_script( 'jquery-infinitescroll', 'https://unpkg.com/infinite-scroll@3/dist/infinite-scroll.pkgd.min.js', array( 'jquery', 'masonry' ), '', true );
...
if ( ! function_exists( 'fukasawa_load_style' ) ) {
	...
	function fukasawa_load_style() {
		...
		if ( ! is_admin() ) {
			...
			wp_enqueue_style( 'fukasawa_style', get_stylesheet_uri(), array( 'fukasawa_googleFonts', 'fukasawa_genericons' ) );
		}
...
$my_posts = get_posts( array('post_type' => 'artworks', 'numberposts' => -1));
//print_r($my_posts);
//die();
foreach ( $my_posts as $my_post ) {
    $updated_post_info = array(
      'ID'           => $my_post->ID,
		...
		// update custom field specifico con valore predef
		update_field('art_copyright', TRUE, $my_post->ID);
		// update entire post
